...
本教程将介绍在 macOS 环境下部署 OpenClaw，QwenPaw 和 Hermes Agent。如无特殊说明，macOS 系统下需在终端中执行命令。本教程涉及到的软件信息如下：
...
为了更好的进行环境隔离，后续我们使用 Podman 安装不同的智能体框架。运行如下命令安装 Podman：
...
命令会在 `~/.openclaw` 目录下安装 Node 环境和相关依赖。进入 OpenClaw 源代码目录下，运行如下命令安装 OpenClaw：
...
在配置中过程中根据实际情况对模型提供商等选项进行配置。配置如下环境变量来使用宿主机中的 OpenClaw CLI 管理 OpenClaw 容器：
...
在配置中过程中根据实际情况对模型提供商等选项进行配置。运行如下代码启动 Gateway 容器：
