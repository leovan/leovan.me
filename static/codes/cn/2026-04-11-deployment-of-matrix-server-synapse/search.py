...
class SearchWorkerStore(SQLBaseStore):
    def store_search_entries_txn(
        ...
        if isinstance(self.database_engine, PostgresEngine):
            ...
            (event_id, room_id, key, vector, chinese_vector, stream_ordering, origin_server_ts)
            VALUES (?,?,?,to_tsvector('english', ?),to_tsvector('chinese', ?),?,?)
            """
...
class SearchStore(SearchBackgroundUpdateStore):
    ...
    async def search_msgs(
        ...
        if isinstance(self.database_engine, PostgresEngine):
            ...
            sql = """
            SELECT ts_rank_cd(chinese_vector, websearch_to_tsquery('chinese', ?)) AS rank,
            room_id, event_id
            FROM event_search
            WHERE chinese_vector @@  websearch_to_tsquery('chinese', ?)
            """
    ...
    async def search_rooms(
        ...
        if isinstance(self.database_engine, PostgresEngine):
            ...
            FROM event_search
            WHERE chinese_vector @@ websearch_to_tsquery('chinese', ?) AND
            """
            args = [search_query, search_query] + args
    ...
    async def _find_highlights_in_postgres(
        ...
        def f(txn: LoggingTransaction) -> set[str]:
            ...
            for event in events:
                ...
                query = (
                    "SELECT ts_headline('chinese', ?, websearch_to_tsquery('chinese', ?), %s)"
                    % (
