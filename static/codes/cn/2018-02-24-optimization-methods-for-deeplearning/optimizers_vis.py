...
class TrajectoryAnimation2D(animation.FuncAnimation):
    def __init__(
        ...
        **kwargs,
        ...
        self.lines = [
            ax.plot([], [], label=label, lw=2)[0]
            for _, label in zip_longest(paths, labels)
        ]
        self.points = [
            ax.plot([], [], 'o', color=line.get_color())[0] for line in self.lines
        ]

        super(TrajectoryAnimation2D, self).__init__(
            fig,
            self.animate,
...
class TrajectoryAnimation3D(animation.FuncAnimation):
    ...
    def animate(self, i):
        for line, point, path, z_paths in zip(
            self.lines, self.points, self.paths, self.z_paths
        ):
            line.set_data(*path[::, : i * self.steps])
...
def beales_2d_plot(
    x_min=-4.5, x_max=4.5, x_step=0.2, y_min=-4.5, y_max=4.5, y_step=0.2
):
    x, y = np.meshgrid(
...
def beales_2d_animation():
    ...
    paths = [
        np.array(optimizer.get_params_history(np.array([2.0, 1.7]), 3000)).T
...
def beales_3d_animation():
    ...
    return TrajectoryAnimation3D(
        *paths, z_paths=z_paths, labels=methods, ax=ax, steps=10
    )


def saddle_3d_plot(x_min=-2, x_max=2, x_step=0.1, y_min=-2, y_max=2, y_step=0.1):
    x, y = np.meshgrid(
        np.arange(x_min, x_max + x_step, x_step),
        np.arange(y_min, y_max + y_step, y_step),
    )
    ...
    ax.plot_surface(
        x, y, z, rstride=1, cstride=1, edgecolor='none', alpha=0.8, cmap='viridis'
    )
...
def saddle_3d_animation():
    ...
    paths = [
        np.array(optimizer.get_params_history(np.array([-0.5, -1e-4]), 300)).T
        for optimizer in optimizers
    ]
    ...
    return TrajectoryAnimation3D(
        *paths, z_paths=z_paths, labels=methods, ax=ax, steps=1
    )
...
if __name__ == '__main__':
    ...
    mp4_writer = animation.FFMpegWriter(
        fps=60, bitrate=1800, metadata=dict(artist='Mr. Black')
    )
