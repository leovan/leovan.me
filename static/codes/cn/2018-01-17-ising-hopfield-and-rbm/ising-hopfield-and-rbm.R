...
diff_steps_matrix <- ising_simulation(
  100, 5000, ising_config,
  c(0, 1, 5, 50, 500, 5000)
)
...
png(
  filename = "../../static/images/cn/2017-12-25-ising-hopfield-and-rbm/digits.png",
...
digits <- lapply(0:9, function(num) {
  readImage(paste0(
    "../../static/images/cn/2017-12-25-ising-hopfield-and-rbm/",
    num, ".png"
  ))
})
...
png(
  filename = "../../static/images/cn/2017-12-25-ising-hopfield-and-rbm/digits-test.png",
...
digits_test_results_patterns <- lapply(
  digits_test_patterns,
  function(pattern) {
...
# 转换测试数据结果为图片
digits_test_results <- lapply(
  digits_test_results_patterns,
  function(result) {
...
png(
  filename = "../../static/images/cn/2017-12-25-ising-hopfield-and-rbm/digits-test-results.png",
  width = 6, height = 1, units = "cm", res = 150
)
old_par <- par()
par(mar = c(1, 1, 1, 1))
...
saveGIF(
  {
    ...
    for (i in seq(steps, 300, by = steps)) {
      ...
      digits_test_results_combine <- combine(digits_test_results)
      display(digits_test_results_combine,
        method = "raster",
  ...
  interval = 0.001,
  movie.name = "digits-test-results.gif",
  ani.width = 354,
  ani.height = 59
)
