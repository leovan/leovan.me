...
bbrbm = BBRBM(
    n_visible=784, n_hidden=64, learning_rate=0.01, momentum=0.95, use_tqdm=True
)
...
bbrbm.save_weights(
    '../../output/cn/2018-01-17-ising-hopfield-and-rbm/mnist-bbrbm', 'default'
)
...
plt.plot(bbrbm_errs)
plt.savefig(
    '../../static/cn/2018-01-17-ising-hopfield-and-rbm/bbrbm-mnist-errs.png',
    bbox_inches='tight',
)
...
for idx in range(mnist_test_images.shape[0]):
    ...
    for digit in range(10):
        ...
        if (label == digit_label).all() and digits_current_counts[digit] < 10:
            ...
            mnist_test_images_samples_rec[sample_idx,] = bbrbm.reconstruct(
                image.reshape([1, -1])
            )
            mnist_test_images_samples_plt[sample_idx * 2,] = mnist_test_images_samples[
                sample_idx,
            ]
...
bbrbm_fig.savefig(
    '../../static/cn/2018-01-17-ising-hopfield-and-rbm/bbrbm-mnist.png',
    bbox_inches='tight',
)
