...
def __mix_color(v1, v2, ratio: float):
    return np.array((1.0 - ratio) * v1 + 0.5).astype(np.uint8) + np.array(
        ratio * v2
    ).astype(np.uint8)
...
def apply_temperature(image, temperature, keep_original_lightness: bool = True):
    ...
    return (
        __keep_original_lightness(image, ret_image)
        if keep_original_lightness
        else ret_image
    )


def apply_kelvin(
    image, kelvin: int, strength: float = 0.6, keep_original_lightness: bool = True
):
    b, g, r = cv2.split(image)
    k_r, k_g, k_b = __kelvin_to_rgb(kelvin)
    n_r, n_g, n_b = (
        __mix_color(r, k_r, strength),
        __mix_color(g, k_g, strength),
        __mix_color(b, k_b, strength),
    )
