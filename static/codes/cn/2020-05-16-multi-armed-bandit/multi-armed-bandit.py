...
class Bandit:
    ...
    def act(self):
        ...
        if self.ucb_param is not None:
            UCB_estimation = self.q_estimation + self.ucb_param * np.sqrt(
...
p = (
    ggplot(df_long)
    + geom_violin(aes(x='action', y='reward_distribution'), draw_quantiles=0.5)
    + xlab('动作')
    + ylab('收益分布')
    + scale_y_continuous(
        limits=[-5, 4],
    ...
    + theme_xkcd()
    + theme(axis_title=element_text(family='Source Han Sans CN', size=10))
)
...
average_rewards_df_long = pd.melt(
    average_rewards_df, ['step'], var_name='action', value_name='average_reward'
)

# %%
p = (
    ggplot(average_rewards_df_long)
    + geom_line(aes(x='step', y='average_reward', color='action'))
    + xlab('训练步数')
    + ylab('平均收益')
    + labs(color='方法')
    + scale_y_continuous(limits=[0, 1.5], breaks=[0, 0.5, 1, 1.5])
    + scale_colour_manual(
...
best_action_counts_df_long = pd.melt(
    best_action_counts_df, ['step'], var_name='action', value_name='best_action_ratio'
)

# %%
p = (
    ggplot(best_action_counts_df_long)
...
p.save(
    'epsilon-greedy-different-parameters-best-action-ratio.png',
    width=6,
    height=4,
    dpi=100,
)
...
p = (
    ...
    + ylab('平均收益')
    + labs(color='方法')
    + scale_y_continuous(limits=[0, 1.6], breaks=[0, 0.5, 1, 1.5])
    + scale_colour_manual(
...
p.save(
    'gradient-different-parameters-best-action-ratios.png', width=6, height=4, dpi=100
)
...
p = (
    ...
    + labs(color='方法')
    + scale_y_continuous(
        limits=[0.75, 1.5], breaks=[0.8, 0.9, 1, 1.1, 1.2, 1.3, 1.4, 1.5]
    )
    + scale_x_continuous(
        limits=[-7, 2],
