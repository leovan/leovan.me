...
# %%
def gen_cavans(
    p_points: List[List[float]], figure_max_width=3, figure_max_height=1.5, dpi=100
):
...
def plot_reference_lines(
    ...
    points_anno_positions: List[List[float]],
    anno_text='P',
...
def plot_bezier_curve(
    ...
    if draw_curve.lower() == 'current':
        ...
        raise ValueError(
            'Unsupprted draw_curve value [{draw_curve}]'.format(draw_curve=draw_curve)
        )
    ...
    plt.annotate(
        r'$B$',
        xy=(bc_points_x[t_idx], bc_points_y[t_idx]),
        xycoords='data',
        xytext=(6, 0),
        textcoords='offset points',
    )
...
def plot_1st_powers_bezier_curve(
    ...
    plot_reference_lines(
        fig, p_points, p_points_anno_positions, anno_text='P', line_width=2
    )
...
def plot_1st_powers_bezier_curve_animation(
    ...
    return animation.FuncAnimation(
        fig, plot_1st_powers_bezier_curve_frame, frames=100, fargs=[100]
    )
...
def plot_2nd_powers_bezier_curve(
    ...
    q_points = [
        get_center_point(p_points[idx], p_points[idx + 1], t)
        for idx in range(len(p_points) - 1)
    ]
    q_points_anno_positions = [[-18, 0], [6, 0]]
    plot_reference_lines(
        fig,
        q_points,
        q_points_anno_positions,
        anno_text='Q',
...
def plot_2nd_powers_bezier_curve_animation(
    ...
    return animation.FuncAnimation(
        fig, plot_2nd_powers_bezier_curve_frame, frames=100, fargs=[100]
    )
...
def plot_3rd_powers_bezier_curve(
    ...
    r_points = [
        get_center_point(q_points[idx], q_points[idx + 1], t)
        for idx in range(len(q_points) - 1)
    ]
    r_points_anno_positions = [[-18, -6], [6, -6]]
    plot_reference_lines(
        fig,
        r_points,
        r_points_anno_positions,
        anno_text='R',
...
def plot_3rd_powers_bezier_curve_animation(
    ...
    return animation.FuncAnimation(
        fig, plot_3rd_powers_bezier_curve_frame, frames=100, fargs=[100]
    )
...
def plot_4th_powers_bezier_curve(
    ...
    s_points = [
        get_center_point(r_points[idx], r_points[idx + 1], t)
        for idx in range(len(r_points) - 1)
    ]
    s_points_anno_positions = [[-18, 0], [6, -6]]
    plot_reference_lines(
        fig,
        s_points,
        s_points_anno_positions,
        anno_text='S',
...
def plot_4th_powers_bezier_curve_animation(
    ...
    return animation.FuncAnimation(
        fig, plot_4th_powers_bezier_curve_frame, frames=100, fargs=[100]
    )
