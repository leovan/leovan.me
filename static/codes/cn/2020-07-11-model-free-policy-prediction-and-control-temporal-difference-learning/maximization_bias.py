...
# %%
# 1,000 runs may be enough, # of actions in state B will also affect the curves
def plot_double_learning_example_left_probability():
    ...
    plt.plot(left_counts_double_q, label='双 Q-Learning')
    plt.plot(np.ones(episodes) * 0.05, label='最优')
    plt.xlabel('幕')
    plt.ylabel('向左动作的概率')
    plt.legend()

    fig.tight_layout()
    fig.savefig('double-learning-example-left-probability.png')


# %%
if __name__ == '__main__':
    plot_double_learning_example_left_probability()
