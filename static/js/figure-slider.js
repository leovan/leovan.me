...
function figureSliderShowImage(
  image,
  baseURL,
  imageFilenamePrefix,
  imageIndex,
...
function figureSliderTogglePlayPause(
  image,
  buttonPlayPause,
  scrollBar,
  figureSliderIndex,
  ...
  if (buttonPlayPause.classList.contains("figure-slider-button-play")) {
    ...
    figureSliderIntervalIDS[figureSliderIndex] = setInterval(
      playNext,
      milliseconds
    );
  } else {
...
function figureSliderPreloadImages(
  baseURL,
  imageFilenamePrefix,
  imageFormat,
  scrollBar
  ...
  for (
    let imageIndex = imageMinIndex;
    imageIndex <= imageMaxIndex;
    imageIndex++
  ) {
    let imageSrc = figureURL(
      baseURL,
      imageFilenamePrefix,
      imageIndex,
      imageFormat
...
(function (d) {
  d.querySelectorAll(".figure-slider").forEach((slider) => {
    ...
    let imageFilenamePrefix = slider.querySelector(
    ...
    let buttonPlayPause = slider.querySelector(
      ".figure-slider-button-play-pause"
    );
    let figureSliderIndex = figureSliderIntervalIDS.length;

    figureSliderIntervalIDS.push(false);

    figureSliderPreloadImages(
      baseURL,
      imageFilenamePrefix,
      imageFormat,
      scrollBar
